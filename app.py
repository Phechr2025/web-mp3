...
FACEBOOK_RE = re.compile(r"^https?://([^/]+\.)?(facebook\.com|fb\.watch)/", re.I)
...
def run_download(job_id, url, fmt, title_override, quality=None, source="youtube"):
    ...
    if fmt == "mp3":
        ...
        if source in ("tiktok", "facebook"):
            # TikTok: เลือกฟอร์แมตจากรายการจริง แบ่งเป็น ต่ำสุด / กลาง / สูงสุด
            probe_opts = {
                "quiet": True,
                "no_warnings": True,
                "noplaylist": True,
            ...
            if not video_fmts:
                raise Exception("No video formats available for this video")
            ...
            # YouTube: เลือกคุณภาพวิดีโอตามระดับที่ผู้ใช้เลือก โดยจำกัดสูงสุดที่ 1080p
            max_h = 1080
            if quality == "medium":
                max_h = 720
            elif quality == "low":
    ...
    # ถ้าเป็นไฟล์วิดีโอ MP4 ให้ฝังรูปปก (thumbnail) ลงในไฟล์ด้วย
    if fmt == "mp4":
...
def api_create():
    ...
    if fmt == "mp4" and quality not in ("low", "medium", "high"):
        quality = "high"
    ...
    elif source in ("tiktok", "facebook"):
        if not TIKTOK_RE.search(url):
            return jsonify({"ok": False, "error": "invalid_url"}), 400
    elif source == "facebook":
        # รองรับลิงก์วิดีโอจาก Facebook / fb.watch
