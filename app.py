...
def run_download(job_id, url, fmt, title_override, quality=None):
    ...
    if fmt == "mp3":
        ...
        # เลือกคุณภาพวิดีโอตามระดับที่ผู้ใช้เลือก โดยจำกัดสูงสุดที่ 1080p
        max_h = 1080
        if quality == "medium":
            max_h = 720
        elif quality == "low":
            max_h = 480
        opts["format"] = f"bestvideo[ext=mp4][height<={max_h}]+bestaudio[ext=m4a]/best[ext=mp4][height<={max_h}]/best[height<={max_h}]"
...
def api_create():
    ...
    t = threading.Thread(target=run_download, args=(job_id, url, fmt, title, quality), daemon=True)
    t.start()
